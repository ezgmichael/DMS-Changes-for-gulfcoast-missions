...
/* Mission System Settings */
	...
	DMS_MissionTimeout					= [1800,2400]; 				// [Minimum,Maximum] time it will take for a mission to timeout | DEFAULT: 15-30 mins
	...
	DMS_MinThrottledDistance			= 1000;						// The minimum distance to which it will throttle. If the throttled value is less than this value, then this value is used instead.
	...
	DMS_RandomBanditMissionsOnStart		= 7;						// Number of (random) bandit missions to spawn when the server starts, just so players don't have to wait for missions to spawn.
	DMS_BanditMissionTypes =			[			//	List of missions with spawn chances. If they add up to 100%, they represent the percentage chance each one will spawn
											...
											["paul_fisher",3],
                                            ["paul_safe_mission",3],
                                            ["paul_industrial_new",3],
                                            ["paul_bandits",3],
                                            ["paul_medical",3],
                                            ["paul_military_camp",3],
                                            //["paul_cargo_tower",3],
                                            ["paul_beer_transport",3],
                                            ["paul_strider_mission",3],
                                            ["paul_church_mission",3],
                                            ["paul_littlebird_mission",3],
                                            ["paul_boat_mission",3],
                                            ["paul_clean_energy",3],
                                            ["paul_ah1z",3],                                  
                                            ["paul_hmmwv",3]
                                            //["paul_sandbags_mission",3],
                                            //["paul_castle",3],                                    
                                            //["paul_c130",3],
                                            //["paul_antiair",3],
                                            //["paul_garage",3]
	...
	DMS_StaticMissionTypes =			[								// List of STATIC missions with spawn chances.
											//["Bushlurker",1]
											//["ISIS",1]		//<--Example (already imported by default on Altis in map configs)
	...
	DMS_BasesToImportOnServerStart = 	[								// List of static bases to import on server startup (spawned post-init). This will reduce the amount of work the server has to do when it actually spawns static missions, and players won't be surprised when a base suddenly pops up. You can also include any other M3E-exported bases to spawn here.
											//"aibase_nwaf"		//<--Example (already imported by default on Altis)
	...
	DMS_findSafePosBlacklist =			[								// This list defines areas where missions WILL NOT spawn. For position blacklist info refer to: http://www.exilemod.com/topic/61-dms-defents-mission-system/?do=findComment&comment=31190
											...
											//[[5567.6035,7665.5879],1000],
											//[[4164.2202,2026.2931],1000],
											//[[8827.6816,6543.9863],1000],
											//[[8697.7461,3063.252],1000],
											//[[2059.9224,6298.9185],1000],
											[[2121.5557,12137.748],1000],
											[[12525.314,5412.0723],1000],
											[[8464.3643,13047.004],1000],
											[[3349.5459,11381.264],1000],
											[[13653.956,8848.6143],1000],
											[[2957.0635,14494.949],1000]
...
/* AI Settings */
	...
	DMS_AI_NamingType					= 1;						// This specifies the "naming scheme" for the AI. 0 corresponds with the default ArmA names; 1 means you want a DMS name (eg: [DMS BANDIT SOLDIER 123]); 2 means you want to generate a name from a list of first and last names (DMS_AI_FirstNames, DMS_AI_LastNames).
	...
	DMS_AIMoney_PopulationMultiplier	= 15;						// This determines how much EXTRA money an AI will have on his body. For example, setting this to 5 and having a server population of 30 means the AI will have an extra 150 poptabs on the body. Set to 0 to disable.
	...
	DMS_Bandit_Static_MoneyGain			= 175;						// The amount of Poptabs gained for killing a bandit static gunner
...
/* DonkeyPunchDMS Custom Settings for Hero AI*/
	DMS_Hero_Soldier_MoneyGain			= 200;						// The amount of Poptabs gained for killing a hero soldier
	...
	DMS_Hero_Static_MoneyGain			= 220;						// The amount of Poptabs gained for killing a hero static gunner
	DMS_Hero_Static_RepGain				= 130;						// The amount of Respect gained for killing a hero static gunner
	...
	DMS_Hero_Vehicle_RepGain			= 150;						// The amount of Respect gained for killing a hero vehicle crew member
...
/* DonkeyPunchDMS Custom Side Factions */
	...
	DMS_survivorSide					= EAST;						// The side (team) that AI Survivor will spawn on
	...
	DMS_ai_freezingDistance				= 5000;						// If there are no players within this distance of the leader of an AI group, then the AI group will be "frozen".
	DMS_ai_unfreezingDistance			= 5000;						// If there are players within this distance of the leader of an AI group, then the AI group will be "un-frozen".
	DMS_ai_offloadOnUnfreeze			= false;						// Whether or not to offload AI to clients once they have been "un-frozen". NOTE: This config will be ignored if "DMS_ai_offload_to_client" is set to false.
	DMS_ai_freezeCheckingDelay			= 0;						// How often (in seconds) DMS will check whether to freeze/un-freeze AI.
	...
	DMS_ai_skill_static					= [["aimingAccuracy",0.10],["aimingShake",0.30],["aimingSpeed",0.15],["spotDistance",0.10],["spotTime",0.40],["courage",1.00],["reloadSpeed",1.00],["commanding",1.00],["general",1.00]];	// Static AI Skills
	DMS_ai_skill_easy					= [["aimingAccuracy",0.10],["aimingShake",0.40],["aimingSpeed",0.20],["spotDistance",0.20],["spotTime",0.40],["courage",1.00],["reloadSpeed",1.00],["commanding",1.00],["general",0.50]];	// Easy
	DMS_ai_skill_moderate				= [["aimingAccuracy",0.20],["aimingShake",0.50],["aimingSpeed",0.25],["spotDistance",0.30],["spotTime",0.60],["courage",1.00],["reloadSpeed",1.00],["commanding",1.00],["general",0.60]];	// Moderate
	DMS_ai_skill_difficult				= [["aimingAccuracy",0.30],["aimingShake",0.60],["aimingSpeed",0.35],["spotDistance",0.40],["spotTime",0.70],["courage",1.00],["reloadSpeed",1.00],["commanding",1.00],["general",0.70]]; 	// Difficult
	DMS_ai_skill_hardcore				= [["aimingAccuracy",0.30],["aimingShake",0.70],["aimingSpeed",0.45],["spotDistance",0.50],["spotTime",0.80],["courage",1.00],["reloadSpeed",1.00],["commanding",1.00],["general",1.00]]; 	// Hardcore
...
/* Loot Settings */
	...
	DMS_ReinforcementHelis =			[							// List of helis that can spawn for AI paratrooper reinforcements.
											"O_T_VTOL_02_infantry_F"		// Ghosthawk: You'll have to whitelist this in infistar if you want to use it.
										] + DMS_TransportHelis;
